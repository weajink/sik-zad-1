...
namespace kayles::parse {
    ...
    template <typename T>
    bool assign_or_report(std::optional<T> &slot,
    ...
    inline std::expected<address_t, KaylesError>
    parse_address(std::string_view address_str) {
        ...
        if (errcode != 0) {
            return std::unexpected(KaylesError::parse_error("getaddrinfo failed: " + std::string(gai_strerror(errcode))));
        }
    ...
    inline std::expected<uint16_t, KaylesError>
    parse_port(std::string_view port_str) {
        uint16_t port;
    ...
    inline std::expected<timeout_t, KaylesError>
    parse_timeout(std::string_view timeout_str) {
        uint8_t seconds;
        auto [ptr, ec] = std::from_chars(
            timeout_str.data(), timeout_str.data() + timeout_str.size(), seconds);
    ...
    inline std::expected<pawn_row_t, KaylesError>
    parse_pawn_row(std::string_view row_str) {
        ...
        for (size_t i = 0; i < row_str.size(); i++) {
            if (row_str[i] != '0' && row_str[i] != '1') {
                return std::unexpected(KaylesError::parse_error("Invalid pawn row format - only '0' and '1' allowed."));
            }
            res[i] = (row_str[i] == '1');
        }

        if (res.front() != true || res.back() != true) {
            return std::unexpected(KaylesError::parse_error("Invalid pawn row format - first and last pawn must be present."));
        }
    ...
    parse_client_message(std::string_view message_str) {
        ...
        while (start <= message_str.size()) {
            ...
            if (end == std::string_view::npos) end = message_str.size();
        ...
        // Parse msg_type.
        uint8_t msg_type;
        auto [ptr_mt, ec_mt] = std::from_chars(
            tokens[0].data(), tokens[0].data() + tokens[0].size(), msg_type);
        ...
        switch (msg_type) {
            case 0: expected_tokens = 2; break;
            case 1:
            case 2: expected_tokens = 4; break;
            case 3:
            case 4: expected_tokens = 3; break;
        ...
        auto [ptr_pid, ec_pid] = std::from_chars(
            tokens[1].data(), tokens[1].data() + tokens[1].size(), msg.player_id);
        ...
        if (msg_type >= 1) {
            auto [ptr_gid, ec_gid] = std::from_chars(
                tokens[2].data(), tokens[2].data() + tokens[2].size(), msg.game_id);
        ...
        if (msg_type == 1 || msg_type == 2) {
            auto [ptr_p, ec_p] = std::from_chars(
                tokens[3].data(), tokens[3].data() + tokens[3].size(), msg.pawn);
