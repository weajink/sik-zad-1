...
run_client "$PORT" "0/7"
assert_exit_code 0
assert_stdout_contains "Player A: 7$"
assert_stdout_contains "Status: waiting for opponent"

run_client "$PORT" "0/7"
assert_exit_code 0
assert_stdout_contains "Player A: 7$"
assert_stdout_contains "Player B: 7$"
assert_stdout_contains "Status: player B's turn"
SAME_GAME_ID=$(echo "$CLIENT_STDOUT" | grep -oE "Game [0-9]+" | head -1 | awk '{print $2}')
...
assert_exit_code 0
assert_stdout_contains "Status: waiting for opponent"
GAME_2_ID_WAIT=$(echo "$CLIENT_STDOUT" | grep -oE "Game [0-9]+" | head -1 | awk '{print $2}')
...
assert_stdout_contains "Status: player B's turn"
assert_stdout_contains "Player A: 7$"
assert_stdout_contains "Player B: 8$"
GAME_2_ID=$(echo "$CLIENT_STDOUT" | grep -oE "Game [0-9]+" | head -1 | awk '{print $2}')
