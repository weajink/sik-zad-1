...
run_client "$PORT" "0/7"
assert_exit_code 0
assert_stdout_contains "Player A ID: 7"
assert_stdout_contains "Status: 0"

run_client "$PORT" "0/7"
assert_exit_code 0
assert_stdout_contains "Player A ID: 7"
assert_stdout_contains "Player B ID: 7"
assert_stdout_contains "Status: 2"
SAME_GAME_ID=$(echo "$CLIENT_STDOUT" | grep "Game ID:" | head -1 | awk '{print $3}')
...
assert_stdout_contains "Pawn Row: 0111"

# Now A's turn — same player_id.
run_client "$PORT" "1/7/$SAME_GAME_ID/1"
assert_exit_code 0
assert_stdout_contains "Status: 2"
assert_stdout_contains "Pawn Row: 0011"
...
assert_exit_code 0
assert_stdout_contains "Status: 0"
GAME_2_ID_WAIT=$(echo "$CLIENT_STDOUT" | grep "Game ID:" | head -1 | awk '{print $3}')
...
assert_stdout_contains "Status: 2"
assert_stdout_contains "Player A ID: 7"
assert_stdout_contains "Player B ID: 8"
GAME_2_ID=$(echo "$CLIENT_STDOUT" | grep "Game ID:" | head -1 | awk '{print $3}')
...
assert_stdout_not_contains "Status: 255"
assert_stdout_contains "Status: 2"
assert_stdout_contains "Pawn Row: 1111"
