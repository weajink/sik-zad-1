...
static Run run_cmd(const std::string& path, const std::vector<std::string>& args,
    ...
    while (true) {
        if (out_open) {
            ...
            if (n > 0) res.stdout_s.append(buf, static_cast<size_t>(n));
            else if (n == 0) out_open = false;
        }
        if (err_open) {
            ssize_t n = ::read(err_pipe[0], buf, sizeof(buf));
            if (n > 0) res.stderr_s.append(buf, static_cast<size_t>(n));
...
TEST(CliClientTimeout, NoServerRespondsInTimeoutButExitZero) {
    ...
    EXPECT_EQ(r.exit_code, 0)
        << "spec: no response → print message and exit 0; got " << r.exit_code;
    EXPECT_FALSE(r.stdout_s.empty())
        << "client must print something on timeout";
}
...
TEST(CliClientSuccess, JoinAgainstRealServerExitsZeroAndPrintsGameState) {
    ...
    EXPECT_EQ(r.exit_code, 0);
    EXPECT_FALSE(r.stdout_s.empty())
        << "client must write received GameState to stdout";
}
